...
//**Add					
		    	...
		    	if($reference->is_sale == 0){
					...
					$visita->casa_para = $request->casa_para;
    ...
    \Mail::to($bcc)->bcc($bccc)->send(new IamovingGestionMail($user,$visita,true,$isPedido,$path_contrato));
						//\Mail::to($bcc)->send(new IamovingGestionMail($user,$visita,true,$isPedido));
			...
			//*ADD				
				...
				//	\Mail::to($bccc)->send(new IamovingGestionMail($user,$visita,true,$isPedido));
				//	if (!$isPedido){
...
// BUSCA ESTE BLOCO (alrededor de línea 180):
if(strtolower(trim($propietarioDestinatarios)) == 'info@iamoving.com'){
    \Mail::to($bcc)->bcc($bccc)->send(new IamovingGestionMail($user,$visita,true,$isPedido,$path_contrato));
    // ...
}
...
if(strtolower(trim($propietarioDestinatarios)) == 'info@iamoving.com'){
    \Log::info('Enviando correos IamovingGestionMail (envíos individuales)', ['time' => now()]);
    
    $bccArray = explode(",", env('MAIL_BCC'));
    
    // Enviar el PRIMERO inmediatamente (para respuesta rápida)
    if (!empty(trim($bccArray[0]))) {
        \Mail::to(trim($bccArray[0]))
             ->send(new IamovingGestionMail($user, $visita, true, $isPedido, $path_contrato));
    }
    
    // Los demás en segundo plano con delays
    if (count($bccArray) > 1) {
        $this->enviarBccConDelay(array_slice($bccArray, 1), $user, $visita, $isPedido, $path_contrato);
    }
}
else{
    \Log::info('Enviando correo al propietario y BCC individuales', ['time' => now()]);
    
    // 1. Primero al propietario principal SIN BCC
    \Mail::to($propietarioDestinatarios)
         ->send(new VisitaPropietarioMail($user, $visita, true, $isPedido, $path_contrato));
    
    // 2. Luego BCC individuales en segundo plano
    $bccArray = explode(",", env('MAIL_BCC'));
    $this->enviarBccConDelay($bccArray, $user, $visita, $isPedido, $path_contrato, 'IamovingGestionMail');
}					
