...
use App\Mail\HalfMail;
use App\Mail\HalfAceptaMail;
use Carbon\Carbon;
...
class HalfController extends Controller
{
	    ...
	    public function show($id) 
    {
        ...
        if($service->estado!=null){
            return view('web.halfjornada.show', compact('service'));    
        }else{
    ...
    {
        return view('web.halfjornada.index');        
    }
    ...
    public function send(Request $request)
    {
		...
		//Grabar
			...
			$colaborador->dni = $dni;
			$colaborador->halfjornada = $halfjornada;
            ...
            $subject = "Solicitud de colaboración Comercial Audiovisual a half objetivos con IAMOVING";
            \Mail::to($mail_to)->bcc($bcc)->send(new HalfMail($dni,$subject,$name,$email,$phone,$address,$avatar,$nif_image,$nif_image2,$halfjornada));
			
			\Mail::to($email)->send(new HalfAceptaMail($dni,$subject,$name,$email,$phone,$address,$avatar,$nif_image,$nif_image2,$halfjornada));
            
            if(count(\Mail::failures()) > 0 ) {
                return redirect()->route('halfobjetivos')->with('error','No se ha podido enviar su mensaje');
            }else{
                return redirect()->route('halfobjetivos')->with('success','Su mensaje ha sido enviado');
            }
        }else{
            return redirect()->route('halfobjetivos')->with('error','Este email ya está dado de alta');
        }
