...
use App\Mail\AudiovisualMail;
use App\Mail\AudiovisualAceptaMail;
use Carbon\Carbon;
...
class AudiovisualController extends Controller
{
	    ...
	    public function show($id) 
    {
        ...
        if($service->estado!=null){
            return view('web.audiovisual.show', compact('service'));    
        }else{
    ...
    {
        return view('web.audiovisual.index');        
    }
    ...
    public function send(Request $request)
    {
		...
		//Grabar
			...
			$colaborador->dni = $dni;
			$colaborador->audiovisual = $audiovisual;
            ...
            $subject = "Solicitud de colaboración Comercial Audiovisual con IAMOVING";
            \Mail::to($mail_to)->bcc($bcc)->send(new AudiovisualMail($dni,$subject,$name,$email,$phone,$address,$avatar,$nif_image,$nif_image2,$audiovisual));
			
			\Mail::to($email)->send(new AudiovisualAceptaMail($dni,$subject,$name,$email,$phone,$address,$avatar,$nif_image,$nif_image2,$audiovisual));
            
            if(count(\Mail::failures()) > 0 ) {
                return redirect()->route('audiovisual')->with('error','No se ha podido enviar su mensaje');
            }else{
                return redirect()->route('audiovisual')->with('success','Su mensaje ha sido enviado');
            }
        }else{
            return redirect()->route('audiovisual')->with('error','Este email ya está dado de alta');
        }
