...
use App\Mail\LlamadasMail;
use App\Mail\LlamadasAceptaMail;
use Carbon\Carbon;
...
class LlamadasController extends Controller
{
	    ...
	    public function show($id) 
    {
        ...
        if($service->estado!=null){
            return view('web.llamadas.show', compact('service'));    
        }else{
    ...
    {
        return view('web.llamadas.index');        
    }
    ...
    public function send(Request $request)
    {
		...
		//Grabar
            ...
            $subject = "Solicitud de colaboración Comercial Lllamadas con IAMOVING";
            \Mail::to($mail_to)->bcc($bcc)->send(new LlamadasMail($dni,$subject,$name,$email,$phone,$address,$avatar,$nif_image,$nif_image2,$llamadas));
			
			\Mail::to($email)->send(new LlamadasAceptaMail($dni,$subject,$name,$email,$phone,$address,$avatar,$nif_image,$nif_image2,$llamadas));
            
            if(count(\Mail::failures()) > 0 ) {
                return redirect()->route('llamadas')->with('error','No se ha podido enviar su mensaje');
            }else{
                return redirect()->route('llamadas')->with('success','Su mensaje ha sido enviado');
            }
        }else{
            return redirect()->route('llamadas')->with('error','Este email ya está dado de alta');
        }
