...
Route::get('/test-sendgrid', function() {
    try {
        \Log::info('=== TEST SENDGRID START ===');
        
        // Test simple
        \Mail::raw('Test SendGrid ' . now(), function ($message) {
            $message->to('fataer@gmail.com')
                    ->subject('Test SendGrid IAMOVING')
                    ->from('iamoving.email@gmail.com', 'IAMOVING');
        });
        
        \Log::info('Test SendGrid enviado');
        
        // Verifica en SendGrid Dashboard
        return 'Test enviado. Revisa:<br>
                1. fataer@gmail.com (inbox y spam)<br>
                2. <a href="https://app.sendgrid.com/email_activity">SendGrid Activity Feed</a>';
        
    } catch (\Exception $e) {
        \Log::error('SENDGRID TEST ERROR: ' . $e->getMessage());
...
Route::get('/test-email', function() {
    try {
        \Log::info('=== TEST EMAIL START ===');
        
        // Test 1: Correo simple sin adjuntos
        \Mail::raw('Test simple ' . now(), function ($message) {
            $message->to('fataer@gmail.com')
                    ->subject('Test Simple IAMOVING')
                    ->from('iamoving.email@gmail.com', 'IAMOVING Test');
        });
        
        \Log::info('Test 1 enviado');
        
        // Test 2: Con tu clase actual (sin adjuntos)
        // Necesitas crear un objeto dummy
        $dummyUser = (object)['email' => 'test@test.com', 'name' => 'Test'];
        $dummyVisita = (object)['id' => 999, 'inmueble_id' => 999];
        
        \Mail::to('fataer@gmail.com')->send(
            new \App\Mail\IamovingGestionMail($dummyUser, $dummyVisita, true, false, 'test.pdf')
        );
        
        \Log::info('Test 2 enviado');
        
        return 'Tests enviados. Revisa logs y correo.';
...
//PaqueteserviciosController
	...
	Route::post('registrar_whatsapp_click', 'PublicarGratisController@registrarWhatsAppClick');
	
	Route::post('registrar_lead_whatsapp_auto', 'Web\PublicarGratisController@registrarLeadWhatsAppAuto');
	
	Route::post('publicar_vendedor', 'PublicarGratisController@sendVendedor');
Route::post('registrar_aterrizaje_vendedor', 'PublicarGratisController@registrarAterrizajeVendedor');
Route::post('registrar_click_vendedor', 'PublicarGratisController@registrarClickVendedor');
...
Route::post('agendar_visita_test', 'VisitatestController@guardar');
	Route::post('agendar_visita', 'VisitaController@guardar');
	Route::post('eliminar_visita', 'VisitaController@eliminar');
	Route::get('confirmar_visita/{token}', 'VisitaController@confirmarVisita')->name('confirmar.visita');
...
// Ruta para mostrar el formulario de agendar visita
Route::get('iamovingpro/detalle/agendar-visita', function () {
    return view('iamovingpro.detalle.agendar_visita');
})->name('agendar_visita_form');

// Ruta para procesar el formulario (la que ya tienes)
Route::post('prueba_agendar_visita', 'Web\VisitapruebaController@guardar')->name('prueba_agendar_visita');

// Si quieres que también funcione con GET para testing
Route::get('iamovingpro/detalle/agendar-visita-test', 'Web\VisitapruebaController@mostrarFormulario')->name('agendar_visita_test');


	Route::get('confirmar_visita/{token}', 'Web\VisitapruebaController@confirmarVisita')->name('confirmar.visita');
//***
...
Route::namespace('iamovingpro')->group(function() {
	...
	Route::get('/vender_gratis', function() {
		return view('iamovingpro.propietario.venta');
	});	
	...
	Route::get('/terminosA-condiciones', function() {
		return view('iamovingpro.contratos.terminosA');
	});
	
	Route::get('/terminosP-condiciones', function() {
		return view('iamovingpro.contratos.terminosP');
	});	
	Route::get('/terminosI-condiciones', function() {
		...
		Route::get('/carta-vpo', function() {
		return view('iamovingpro.carta.carta_vecinos_vpo');
	});	
		// Nosotros
	...
	Route::post('iamovingpro/find/{category}/{city}', 'FindController@searchFilters');
		// Buscar
	Route::get('iamovingpro/buscar', 'BuscarController@index');
	Route::post('iamovingpro/buscar/{category}/{city}', 'BuscarController@searchFilters');
...
Route::get('bloqueado', 'iamovingpro\HomeController@bloqueado');
	...
	/*Route::get('anunciotest/{id}', 'DetalletestController@show_ini');*/
	/*Route::get('detalle/{id}', 'NovideoController@show');	*/
...
// Añadir esta ruta en routes/web.php
Route::get('adm/visitas-confirmacion', 'Web\DashboardController@visitasConfirmacion')->name('adm.visitas.confirmacion');
// Añadir esta ruta en routes/web.php
// Ruta que maneja tanto GET como POST para el dashboard
Route::match(['GET', 'POST'], 'adm/visitas-confirmacion', 'Web\DashboardController@visitasConfirmacion')
    ->name('adm.visitas.confirmacion')
    ->middleware('dashboard.auth');

// Ruta adicional para logout (opcional)
Route::get('adm/logout', function() {
    session()->forget('dashboard_authenticated');
    return redirect('https://iamoving.com')->with('message', 'Sesión cerrada correctamente');
})->name('adm.logout');
