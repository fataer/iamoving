    ...
    <Modal>
        <template v-slot:content>
                ...
                <div v-if="mode == 1 || mode == 2">
                    <nav class="nav nav-pills nav-justified" role="tablist">
                        <a href="#" class="nav-item nav-link"
                            :class="{ active: mode == 1 }"
                            @click.prevent="mode = 1">Log in</a>
                        <a href="#" class="nav-item nav-link"
                            :class="{ active: mode == 2 }"
                            @click.prevent="mode = 2">Aren't you registered?</a>
                    </nav>
                </div>
                <br>
            ...
            <div class="text-center terms" v-if="mode == 1">
                By logging in you are accepting the <a href="/terminosC-condiciones" target="_blank">terms and conditions</a> and the <a href="/politica-privacidad" target="_blank">policy privacy</a> of <b>IAMOVING</b>
            </div>
            <div class="text-center terms" v-if="mode == 2">
               By registering you are accepting the <a href="/terminosC-condiciones" target="_blank">terms and conditions</a> and the <a href="/politica-privacidad" target="_blank">policy privacy</a> of <b>IAMOVING</b>
            </div>			
            <div class="form-group" v-if="mode == 1|| mode == 2">
                <button type="button" class="btn btn-link btn-block" v-on:click="retrieve">FORGOT YOUR PASSWORD?</button>
            </div>
        </template>
    </Modal>
</template>
    ...
    methods: {
            ...
            axios.post('/auth/register', request)
                .then((response) => {
                    if (response.status == 200) {
                        //this.login(request);
                        this.feedback("You have successfully registered. Enter your Email and Password to log in. Thanks", false);
                        this.mode = 1;
                    }else{
                        console.log("No");
        ...
        extractError(response){
            if(response.status == 422){
                ...
                this.feedback("Email already registered. Sign in or go to Forgot your password?", true);
            }
            ...
            if(valid){
                ...
                axios.post('/auth/reset', request)
                    .then((response) => {
                        if (response.status == 200) {
                            console.log(response);
                            this.feedback("Your password has been reset", false);
                            this.mode = 1;
                        }
                    })
                    .catch((exception) => {
                ...
                this.feedback("Passwords must match", true);
            }
        }
    }
}
