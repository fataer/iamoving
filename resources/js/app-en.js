...
Vue.component('show-case-en', require('./components/ShowCase-en/ShowCase.vue').default);
...
// Containers
Vue.component('modal-auth', require('./containers/ModalAuth-en').default);
...
Vue.component('report-content', require('./containers/ReportContent-en').default);
//Vue.component('report-basico', require('./containers/ReportBas').default);
Vue.component('report-lugar-en', require('./containers/ReportLugar-en').default);
...
const app = new Vue({
                 ...
                 axios.post('/comprar_alquilar', {user_id:this.user.id, reference: reference})
                     .then((response) => {
                         ...
                         if (response.status == 200) {
                             Swal.fire(
                                 'Thanks',
                                 'Your request has been processed, we will contact you',
                                 'success'
                             );
                         }
                     })
                 ...
                 axios.post('/auth/resend', {id:this.user.id})
                     .then((response) => {
                         ...
                         if (response.status == 200) {
                             Swal.fire(
                                 'Thanks',
                                 'Your request has been processed, we have re-sent you the verification email',
                                 'success'
                             );
                         }
                     })
		...
		console.log("pedidoSubmited4.5");
          if (response.data.success) {
            ...
            Swal.fire({
              ...
              html: '<p>Solicitud enviada<span style=font-family:Segoe UI Emoji,sans-serif;mso-bidi-font-family:Segoe UI Emoji>&#128578;</span></p><p>We will confirm you as soon as possible, since we need the approval of the owner to confirm the counter offer.</p>'
            });
          } else {
            Swal.fire('Warning', 'You already have a visit scheduled for this property', 'info');
          }
        }
      })
                    .catch((exception) => {
                            ...
                            if(response.data.success){
								...
								if (this.user){
                                 ...
                                 Swal.fire({
								  ...
								  html: '<p>Solicitud enviada<span style=font-family:Segoe UI Emoji,sans-serif;mso-bidi-font-family:Segoe UI Emoji>&#128578;</span></p><p>We will confirm you as soon as possible, since we need the approval of the owner to confirm the appointment.</p>'
                                 ...
                                 Swal.fire(
                                     'Warning',
                                     'You already have a visit scheduled for this property',
                                     'info'
                                 );    
                             }
         ...
         modalVisita(){
             ...
             if(this.is_sale){
				...
				if(this.collapse_data.length === this.total_collapse_data){					
                    ...
                    let htmlMsg = '<p><span style=font-weight:bold;>In order to request a visit you must view the following contents:</span></p>';
         ...
         modalPedido(){
            if(this.is_sale){
                if(/*this.informe_foto && this.informe_barrio && this.informe_mapa && */
                    ...
                    let htmlMsg = '<p><span style=font-weight:bold;>In order to make a counteroffer you must view the following contents:</span></p>';
                    htmlMsg += this.buildHtmlModal();
  ...
  iconColor: "#ff6600",					*/
		        ...
		        $("#modalPedido").modal('show');
                    ...
                    let htmlMsg = '<p><span style=font-weight:bold;>In order to make a counteroffer, you must view the following content:</span></p>';
                    htmlMsg += this.buildHtmlModal();
